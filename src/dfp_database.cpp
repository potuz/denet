...
Dfp::Company Dfp::Database::get_company_from_cvm ( int cvm ) {
  ...
  return Dfp::Company ( cvm, res->getString(1), res->getString(2),
      ...
      res->getString(9), shared_from_this()); 
};
int Dfp::Database::get_cvm_from_ticker_str ( const std::string & str ) {
  std::unique_ptr< sql::Statement> stmt ( conn->createStatement());
  stmt->execute ( "USE denet" );
  stmt.reset(NULL);
  std::unique_ptr<sql::PreparedStatement> prep_stmt (
...
int Dfp::Database::last_imported_revision (int cvm, std::tm tm) {
  std::unique_ptr< sql::Statement> stmt ( conn->createStatement());
  stmt->execute ( "USE denet" );
...
void Dfp::Database::add_revision (int cvm, int revision, std::tm tm ) { 
  std::unique_ptr< sql::Statement> stmt ( conn->createStatement());
  stmt->execute ( "USE denet" );
  std::string date_str = tm_to_string ( tm );
...
void Dfp::Database::delete_exercise (int cvm, std::tm tm ) { 
  std::unique_ptr< sql::Statement> stmt ( conn->createStatement());
  stmt->execute ( "USE denet" );
...
std::tm Dfp::Database::last_imported_exercise ( int cvm ) {
  std::unique_ptr< sql::Statement> stmt ( conn->createStatement());
  stmt->execute ( "USE denet" );
  std::unique_ptr< sql::ResultSet> res ( stmt->executeQuery ( 
...
void Dfp::Database::create_table (int cvm ) { 
  std::unique_ptr< sql::Statement> stmt ( conn->createStatement());
  stmt->execute ( "USE denet" );
