 ...
 *
 *  Copyleft (c) 2016 potuz@potuz.net
 *
...
namespace Dfp {
  ...
  class Database : public std::enable_shared_from_this<Database> {
    ...
    public:
      ...
      Dfp::Company get_company_from_cvm ( int cvm );
      int get_cvm_from_ticker_str ( const std::string & str );
      Dfp::Company get_company_from_ticker ( const Dfp::Ticker& ticker );
      void setup_company ( Dfp::Company& company, int cvm );
      void setup_company ( Dfp::Company& company, 
          const std::string& ticker_code );
      void setup_company ( Dfp::Company& company, 
          const Dfp::Ticker& ticker );
      void setup_company ( Dfp::Company& company );

      std::string get_ticker_on_from_cvm ( int cvm );
      std::vector<std::string> get_tickers_from_cvm ( int cvm );

      int last_imported_revision ( int cvm, std::tm tm );
      std::tm last_imported_exercise ( int cvm );
      void add_revision (int cvm, int revision, std::tm tm );
      void delete_exercise (int cvm, std::tm tm );

      void create_table ( int cvm );
